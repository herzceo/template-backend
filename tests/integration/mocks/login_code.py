...
@final
@dataclass
class MockLoginCodeStore:
    """In-memory ``LoginCodeStore`` — mirrors ``MockVerificationCodeStore``.

    A distinct store from email verification (own dict, own attempt counters), so a
    login code and a verification code for the same user never collide.
    """
