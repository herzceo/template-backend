...
# caveman state is project-local: this script lives in <project>/.claude/hooks,
# so its parent dir is the project .claude. Override with CAVEMAN_STATE_DIR.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STATE_DIR="${CAVEMAN_STATE_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
...
if [ "${CAVEMAN_STATUSLINE_SAVINGS:-1}" != "0" ]; then
  SAVINGS_FILE="$STATE_DIR/.caveman-statusline-suffix"
