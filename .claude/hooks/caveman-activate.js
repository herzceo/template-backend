...
// caveman state (flag/history/suffix) is project-local: __dirname is
// <project>/.claude/hooks, so '..' is the project's .claude dir. Override with
// CAVEMAN_STATE_DIR. This keeps caveman on/off + stats scoped per-project
// instead of shared machine-wide via ~/.claude.
const stateDir = process.env.CAVEMAN_STATE_DIR || path.join(__dirname, '..');
const flagPath = path.join(stateDir, '.caveman-active');
...
// Check the global config-dir settings AND the project-local settings
// (__dirname is .claude/hooks, so ../settings.json is the project .claude
// settings). For project-local installs the statusLine lives in the project
// settings, which Claude Code merges over the global one — without this the
// nudge would fire every session even though the badge is already configured.
try {
  let hasStatusline = false;
  const settingsCandidates = [settingsPath, path.join(__dirname, '..', 'settings.json')];
  for (const candidate of settingsCandidates) {
    if (hasStatusline) break;
    try {
      if (!fs.existsSync(candidate)) continue;
      const settings = JSON.parse(fs.readFileSync(candidate, 'utf8'));
      if (settings.statusLine) hasStatusline = true;
    } catch (e) { /* unreadable/invalid candidate — try the next */ }
  }
