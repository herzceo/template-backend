...
function main() {
  ...
  // claudeDir (~/.claude) is used ONLY to scan Claude Code's own session
  // transcripts under projects/. caveman's own state (history/flag/suffix) is
  // project-local in stateDir. __dirname is <project>/.claude/hooks; '..' is the
  // project .claude. Override: CAVEMAN_STATE_DIR.
  const stateDir = process.env.CAVEMAN_STATE_DIR || path.join(__dirname, '..');
  const historyPath = path.join(stateDir, '.caveman-history.jsonl');
  ...
  const mode = readFlag(path.join(stateDir, '.caveman-active'));
  ...
  if (parsed.turns > 0) {
    ...
    safeWriteFlag(path.join(stateDir, '.caveman-statusline-suffix'), suffix);
  }
