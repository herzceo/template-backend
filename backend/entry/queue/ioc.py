...
def create_email_provider(resend_config: ResendConfig, from_email: str) -> Provider:
    provider = Provider(scope=Scope.APP)
    session = create_aiohttp_session(AiohttpConfig(BASE_URL=resend_config.RESEND_BASE_URL))
    client = ResendClient(session=session, config=resend_config)
