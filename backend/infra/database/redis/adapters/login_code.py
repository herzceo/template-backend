...
from backend.infra.database.redis.adapters.config import LoginCodeConfig
...
@final
class ImplRedisLoginCodeStore(LoginCodeStore):
    __slots__ = ("_config", "_hasher", "_redis")

    def __init__(
        self, redis: RedisClient, hasher: SecretTokenGenerator, config: LoginCodeConfig
    ) -> None:
        self._redis = redis
        self._hasher = hasher
        self._config = config
    ...
    async def issue_code(self, user_id: UUID) -> str:
        ...
        await self._redis.set_hash(
            ...
            ttl=self._config.LOGIN_CODE_TTL_SECONDS,
        )
        return raw_code
    ...
    def max_attempts(self) -> int:
        return self._config.LOGIN_CODE_MAX_ATTEMPTS

    @property
    def ttl_seconds(self) -> int:
        return self._config.LOGIN_CODE_TTL_SECONDS
